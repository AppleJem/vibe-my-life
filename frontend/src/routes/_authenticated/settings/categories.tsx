...
function ConfigureCategoriesPage() {
  ...
  useEffect(() => {
    if (!loading) setDraft(toDraft(categories))
  }, [loading, categories])
  ...
  const handleSave = async () => {
    ...
    const next: Category[] = draft.map((c) => ({
      name: c.name.trim(),
      subcategories: c.subcategories.map((s) => s.name.trim()),
    }))

    // Only renames of nodes that already existed are applied retroactively.
    // Removed nodes emit nothing, so past expenses keep their old category.
  ...
  return (
    <div className="pb-24">
      ...
      <p className="text-sm text-zinc-500 mb-6 pl-1">
        Renaming updates all existing expenses. Deleting leaves them untouched.
      </p>
          ...
          <div key={cat.id} className="bg-zinc-900 rounded-xl p-3">
            <div className="flex items-center gap-2">
              <input
                ...
                className="flex-1 bg-zinc-800 rounded-lg px-3 py-2 text-zinc-100 placeholder-zinc-600 outline-none focus:ring-2 focus:ring-rose-400"
              />
              <button
            ...
            <div className="mt-2 pl-4 space-y-2 border-l border-zinc-800 ml-1">
                ...
                <div key={sub.id}>
                  <div className="flex items-center gap-2">
                    <input
                      ...
                      className="flex-1 bg-zinc-800 rounded-lg px-3 py-1.5 text-sm text-zinc-100 placeholder-zinc-600 outline-none focus:ring-2 focus:ring-rose-400"
                    />
                    <button
              ...
              <button
                ...
                className="text-xs text-zinc-500 hover:text-rose-400 transition-colors py-1"
              >
                + Add subcategory
              </button>
            </div>
      ...
      <button
        onClick={addCategory}
        className="w-full mt-4 py-3 rounded-xl border border-dashed border-zinc-700 text-zinc-400 hover:text-rose-400 hover:border-rose-400 transition-colors"
      >
        + Add category
      </button>
