/**
 * Creates the DynamoDB table the Climbing app stores sessions in.
 *
 ...
 *
 * The schema matches the other life-app tables — `PK` / `SK` strings, on-demand billing,
 * no secondary indexes. Sessions are read by prefix within one user's partition, and there is
...
const TableName = env.CLIMBING_TABLE_NAME
...
async function main() {
  ...
  await client.send(new CreateTableCommand({
    ...
    // On-demand: one person's climbing log has no throughput worth provisioning for.
...
main().catch((err) => {
  console.error('Failed to create the climbing table:', err)
  process.exit(1)
})
